...
drain_compaction() {
	...
	echo "WARNING: compaction still pending after ${COMPACTION_DRAIN_TIMEOUT}s" >&2
}
...
rmdir "$WORKTREE"
SB_ORIGINAL_REPLACE=$(cd "$SB_DIR" && go list -m -f '{{with .Replace}}{{.Path}}{{end}}' github.com/gocql/gocql 2>/dev/null || true)
cleanup() {
	git -C "$GOCQL_DIR" worktree remove --force "$WORKTREE" 2>/dev/null || true
	[ -n "$SB_ORIGINAL_REPLACE" ] &&
