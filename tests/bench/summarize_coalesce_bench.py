...
def collect(d):
    ...
    for name in sorted(os.listdir(d)):
        ...
        if len(parts) < 2:
            continue
        cfg, tier = parts[0], parts[1]
        parsed = parse_run(os.path.join(d, name))
        if parsed:
            runs.setdefault((cfg, tier), []).append(parsed)
    return runs
...
def report(d):
    ...
    label = meta["label"]
    unthrottled = set(meta.get("rates", "").split(",")) and "=0" in meta.get("rates", "")
    objective = "throughput" if unthrottled else "latency"
    ...
    if objective == "throughput":
        print("Objective: throughput and client CPU per op. Latency is not "
              "reported: these runs saturate, so latency is queueing delay.\n")
        print("| Tier | Config | n | ops/s | Δ | CPU µs/op | Δ | Skbs/op |")
        print("|" + "---|" * 8)
    else:
        print("Objective: latency at a fixed offered rate "
    ...
    for tier in TIER_ORDER:
        ...
        b_p50, b_p50_sd = stat([r["co_median"] for r in base])
        b_p99, b_p99_sd = stat([r["co_p99"] for r in base])
        ...
        for cfg in CONFIG_ORDER:
            ...
            if objective == "throughput":
                ...
                p50, p50_sd = stat([r["co_median"] for r in rs])
                p99, p99_sd = stat([r["co_p99"] for r in rs])
                p999, _ = stat([r["co_p999"] for r in rs])
                print(f"| {tier_cell} | {cfg} | {len(rs)} | "
                      f"{ops:,.0f}{flag} | "
