...
func (c *Conn) init(ctx context.Context, dialedHost *DialedHost) error {
	...
	if c.session.cfg.WriteCoalesceWaitTime > 0 && !c.cfg.disableCoalesce && !dialedHost.DisableCoalesce {
		...
		c.w = newWriteCoalescer(dialedHost.Conn, c.cfg.ConnectTimeout, window, coalesceFlushThreshold, ctx.Done())
	}
...
//
// ponytail: fixed default, not MSS-probed per connection. See
// docs/coalescing-redesign.md F8 if that gap ever needs closing.
...
func (w *writeCoalescer) flush(resultChans []chan<- writeResult, buffers net.Buffers) {
	...
	if len(buffers) == 1 {
		...
		if cap(w.scratchBuf) > 4*coalesceFlushThreshold {
			w.scratchBuf = nil
		}
	}
	n := int64(wn)
