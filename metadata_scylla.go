	...
	applied := s.metadata.keyspaceMetadata.updateKeyspace(keyspaceName, func(ks *KeyspaceMetadata) {
		...
		if needAggregates && ks.aggregatesInvalidated && ks.aggregatesInvalidationGen == aggregatesGeneration {
			...
			for i := range aggregates {
				linkAggregateFunctions(&aggregates[i], ks.Functions)
				ks.Aggregates[aggregates[i].Name] = &aggregates[i]
			}
...
// linkAggregateFunctions resolves FinalFunc and StateFunc references in an
// aggregate from the functions map. Missing function references are silently
// left as zero-value FunctionMetadata.
func linkAggregateFunctions(aggregate *AggregateMetadata, functions map[string]*FunctionMetadata) {
	if fn, ok := functions[aggregate.finalFunc]; ok {
		aggregate.FinalFunc = *fn
	}
	if fn, ok := functions[aggregate.stateFunc]; ok {
...
func compileMetadata(
	...
	for i := range aggregates {
		linkAggregateFunctions(&aggregates[i], keyspace.Functions)
