...
// mockSessionForPinTest is a minimal mock for testing nextIter.fetch() fallback logic.
type mockSessionForPinTest struct {
	calls   int
	results []*Iter
}

func (m *mockSessionForPinTest) executeQuery(qry *Query) *Iter {
	idx := m.calls
	m.calls++
...
func TestPinPagesToHost_FallbackOnHostConnError(t *testing.T) {
	// Test the fallback logic: when the pinned host fails with a
	// host-connectivity error, hostID should be cleared and a retry
	// should happen through normal host selection.
	hostErrors := []error{
		ErrHostDown,
		ErrNoPool,
		ErrNoConnectionsInPool,
		ErrConnectionClosed,
		ErrTimeoutNoResponse,
		ErrTooManyTimeouts,
		ErrNoStreams,
		fmt.Errorf("wrapped: %w", ErrHostDown),
		fmt.Errorf("host pool issue: %w", ErrNoPool),
	}

	for _, hostErr := range hostErrors {
		t.Run(hostErr.Error(), func(t *testing.T) {
			failedIter := &Iter{err: hostErr}
			successIter := &Iter{numRows: 5}

			ms := &mockSessionForPinTest{
			...
			if next != nil && next.err != nil && qry.pinPagesToHost && qry.hostID != "" && isHostConnError(next.err) {
				qry.hostID = ""
				next = ms.executeQuery(qry)
			}
...
func TestPinPagesToHost_NoFallbackOnNonHostError(t *testing.T) {
	// When the error is NOT host-related (e.g., query syntax error,
	// permission denied), the fallback should NOT trigger even with
	// pinPagesToHost enabled. The original error should be preserved.
	nonHostErrors := []error{
		errors.New("syntax error in CQL query"),
		errors.New("permission denied"),
		errors.New("keyspace not found"),
	}

	for _, queryErr := range nonHostErrors {
		t.Run(queryErr.Error(), func(t *testing.T) {
			failedIter := &Iter{err: queryErr}
			...
			// Simulate the fallback logic from nextIter.fetch()
			if next != nil && next.err != nil && qry.pinPagesToHost && qry.hostID != "" && isHostConnError(next.err) {
				t.Fatal("fallback should NOT trigger for non-host errors")
			}

			// hostID should remain unchanged — we don't clear it for non-host errors
			if qry.hostID != "pinned-host" {
				t.Fatalf("hostID should remain %q for non-host errors, got %q", "pinned-host", qry.hostID)
			}
			if ms.calls != 1 {
				t.Fatalf("expected 1 executeQuery call (no fallback), got %d", ms.calls)
			}
		})
	}
}
...
func TestPinPagesToHost_NoFallbackWhenNotPinned(t *testing.T) {
	...
	failedIter := &Iter{err: ErrHostDown}
...
func TestPinPagesToHost_PoolRecyclingResetsFlag(t *testing.T) {
	// Verify that reset() properly clears pinPagesToHost so pooled
	// Query objects don't leak the flag to subsequent users.
	q := &Query{routingInfo: &queryRoutingInfo{}, refCount: 1}
	q.PinPagesToHost()
	q.hostID = "some-host"

	if !q.pinPagesToHost {
		t.Fatal("pinPagesToHost should be true before reset")
	}

	q.reset()

	if q.pinPagesToHost {
		t.Fatal("pinPagesToHost should be false after reset()")
	}
	if q.hostID != "" {
		t.Fatalf("hostID should be empty after reset(), got %q", q.hostID)
	}
}

func TestIsHostConnError(t *testing.T) {
	// Verify isHostConnError correctly identifies host-connectivity errors.
	hostErrors := []error{
		ErrHostDown,
		ErrNoPool,
		ErrNoConnectionsInPool,
	...
	for _, err := range hostErrors {
		if !isHostConnError(err) {
			t.Errorf("expected isHostConnError(%v) = true", err)
		}
		// Also test wrapped errors
		wrapped := fmt.Errorf("wrapping: %w", err)
		if !isHostConnError(wrapped) {
			t.Errorf("expected isHostConnError(wrapped %v) = true", err)
		}
	}
	...
	for _, err := range nonHostErrors {
		if err != nil && isHostConnError(err) {
			t.Errorf("expected isHostConnError(%v) = false", err)
		}
	}
}
