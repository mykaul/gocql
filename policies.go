...
// returned by the token-aware iterator. For the common case (RF <= 9), it uses
// an inline array on the stack, avoiding the ~200+ byte map allocation that
// occurred on every query. Sized for NTS with 3 DCs at RF=3 each (9 replicas);
// overflow is handled gracefully by silently stopping tracking (deduplication
// may miss duplicates in that case, but the fallback path remains correct).
type hostSet struct {
	arr [9]*HostInfo
	n   int
}
